...
function buildDocx(resumeData, lang) {
    ...
    const labels = {
        ...
        education: { en: 'Education', 'pt-br': 'Educação' },
    ...
    (resumeData.contact || []).forEach((item) => {
        const label = pickI18n(item.i18n, lang);
        if (item.type === 'link' && item.link) {
            const href = item.link;
            if (window.docx.ExternalHyperlink) {
                paragraphs.push(
                    new Paragraph({
                        children: [
                ...
                paragraphs.push(new Paragraph({ text: `${label}: ${href}`, bullet: { level: 0 } }));
            }
        } else {
            const text = label;
            if (text) {
    ...
    paragraphs.push(new Paragraph({
        text: pickI18n(labels.education, lang),
        heading: HeadingLevel.HEADING_2,
        spacing: { after: 100 },
        color: green
    }));
    (resumeData.education || []).forEach((edu) => {
        const course = pickI18n(edu.i18n, lang);
        const period = `${edu.start} - ${edu.end}`;
        const items = [edu.institution, course, period].filter(Boolean).join(' | ');
        paragraphs.push(new Paragraph({ text: items, bullet: { level: 0 } }));
    });
    ...
    (resumeData.experience || []).forEach((exp) => {
        ...
        if (exp.technologies?.i18n) {
            paragraphs.push(
                new Paragraph({
                    text: `${i18n.translateForKey('experience.technologies', lang) || 'technologies: '} ${pickI18n(exp.technologies.i18n, lang)}`,
                    spacing: { after: 50 }
                })
            );
        }
